...
func (s *apiServer) stubHandlerNotApiError(w http.ResponseWriter, r *http.Request) error {
	return errors.New("Stub Handler Not apiError")
}
func (s *apiServer) stubHandlerApiError(w http.ResponseWriter, r *http.Request) error {
	err := errors.New("Stub Handler apiError")
...
func TestMakeHttpHandler(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputTestUrlPath:      testErrorPath,
			expectedStatusCode:    http.StatusInternalServerError,
			expectedErr:           "Stub Handler Not apiError",
		},
	}
	...
	http.HandleFunc(testApiErrorPath, makeHttpHandler(apiSvc.stubHandlerApiError))
	http.HandleFunc(testErrorPath, makeHttpHandler(apiSvc.stubHandlerNotApiError))
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for attempt := 1; attempt <= maxAttempts; attempt++ {
				resp, err = http.Get(fmt.Sprintf("http://localhost%s%s",
					inputPort, tc.inputTestUrlPath))
				if err == nil {
					httpConn = true
					break
				}
			...
			if apiError.Status != tc.expectedStatusCode {
				t.Fatalf("Test Failed. Expected Result: %v Actual Result: %v",
					tc.expectedStatusCode, resp.StatusCode)
			}

			if apiError.Err != tc.expectedErr {
				t.Fatalf("Test Failed. Expected Result: %v Actual Result: %v",
					tc.expectedErr, resp.StatusCode)
			}
		})
	}
...
func TestHandleRawStats(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := os.Mkdir(fullPathSortKeyServer, 0755); err != nil {
				t.Fatalf("Internal Testing error: %v", err)
			}
			defer func() {
				if err := os.RemoveAll(fullPathSortKeyServer); err != nil {
			...
			resultResponseUrlStats := new(types.ResponseUrlStats)
...
func TestHandleSortKey_limitFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			testResponseUrlStats := new(types.ResponseUrlStats)
